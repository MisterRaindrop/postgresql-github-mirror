 ...
 *
 *
 * Table AMs can use these functions to implement table_index_getnext_slot
 * callbacks described by access/tableam.h.  This includes the table AM side
 * of batch-based index scans.  The scan's batch ring buffer is managed by the
 * table AM, since it is closely tied to whatever mechanism the table AM uses
 * to implement prefetching of table blocks (typically a read stream).
 *
